...
class ResponsePredictor(nn.Module):
    def __init__(self, state_dim=8, representation_predictor_dims=(64, 32), gru_ksz=3,
        ...
        self.init_hidden_state_predictor_exc = nn.Sequential(
                conv_block(1, state_dim, kernel_size=3, stride=1, padding=1, batch_norm=False, relu=False, bias=False),
                nn.Softplus())
        self.init_hidden_state_predictor_inh = nn.Sequential(
                conv_block(1, state_dim, kernel_size=3, stride=1, padding=1, batch_norm=False, relu=False, bias=False),
                nn.Softplus())
    ...
    def forward(self, cost_volume, state_prev, dimp_score_cur, init_label=None, dimp_thresh=None,
        ...
        if state_prev is None:
            init_hidden_state_exc = self.init_hidden_state_predictor_exc(init_label.view(num_sequences, 1,
                                                                                 feat_sz[0], feat_sz[1]))
            init_hidden_state_inh = self.init_hidden_state_predictor_inh(init_label.view(num_sequences, 1,
                                                                                 feat_sz[0], feat_sz[1]))
            state_prev_ndhw_exc = init_hidden_state_exc
            state_prev_ndhw_inh = init_hidden_state_inh
        else:
            state_prev_ndhw_exc, state_prev_ndhw_inh = state_prev

        is_target = self.is_target_predictor(state_prev_ndhw_exc)
        auxiliary_outputs['is_target'] = is_target

        state_prev_ndhw_exc = state_prev_ndhw_exc.view(num_sequences, -1, feat_sz[0], feat_sz[1])
        state_prev_nhwd_exc = state_prev_ndhw_exc.permute(0, 2, 3, 1).contiguous(). \
            view(num_sequences, feat_sz[0] * feat_sz[1], -1, 1, 1).expand(-1, -1, -1, feat_sz[0], feat_sz[1])

        state_prev_ndhw_inh = state_prev_ndhw_inh.view(num_sequences, -1, feat_sz[0], feat_sz[1])
        # state_prev_nhwd_inh = state_prev_ndhw_inh.permute(0, 2, 3, 1).contiguous(). \
        #     view(num_sequences, feat_sz[0] * feat_sz[1], -1, 1, 1).expand(-1, -1, -1, feat_sz[0], feat_sz[1])
        state_prev_nhwd_inh = state_prev_ndhw_inh.permute(0, 2, 3, 1).contiguous()  # . \
        #     view(num_sequences, feat_sz[0] * feat_sz[1], -1, 1, 1).expand(-1, -1, -1, feat_sz[0], feat_sz[1])
        ...
        propagated_h = (propagation_weight_norm * state_prev_nhwd_exc).sum(dim=1)
        ...
        # Pass previous inh state as is
        propagated_h_inh = state_prev_nhwd_inh.view(num_sequences, -1, feat_sz[0], feat_sz[1])
        ...
        state_new, state_inh = self.state_predictor(input=state_gru_input, state_cur_exc=propagated_h, state_cur_inh=propagated_h_inh)

        is_target_new = self.is_target_predictor(state_new)
        auxiliary_outputs['is_target_new'] = is_target_new

        # Put exc + inh states into a list
        state_new = [state_new, state_inh]
